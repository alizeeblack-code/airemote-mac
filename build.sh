...
BIN="$(swift build -c release --arch arm64 --arch x86_64 --show-bin-path)/AIRemoteMac"
...
import re
s = open("Sources/AIRemoteMac/RemoteUI.swift", encoding="utf-8").read()
...
echo "▸ 检查地址分类"
swiftc -O Sources/AIRemoteMac/NetAddress.swift tools/nettest/main.swift \
  -o /tmp/joycoding-nettest 2>/dev/null
/tmp/joycoding-nettest || { echo "❌ 地址分类有回归"; exit 1; }

echo "▸ 测 DualSense 报告解析"
swiftc -O Sources/AIRemoteMac/DualSenseRaw.swift tools/dualsense-test/main.swift \
       -o /tmp/jc-ds-test 2>/dev/null && /tmp/jc-ds-test || { echo "❌ DualSense 解析用例失败"; exit 1; }

echo "▸ 检查手机页 L()"
python3 - <<'CHK' || exit 1
import re, sys
src = open("Sources/AIRemoteMac/RemoteUI.swift", encoding="utf-8").read()
...
src = open('Sources/AIRemoteMac/Translations.swift', encoding='utf-8').read()
...
miss = []
for f in glob.glob('Sources/AIRemoteMac/**/*.swift', recursive=True):
...
_src = open('Sources/AIRemoteMac/Translations.swift', encoding='utf-8').read()
...
cp "$BIN" "$APP/Contents/MacOS/AIRemoteMac"

# 两个切片都在, 才算通用包 —— 少一个的话装到 Intel 机上直接打不开
ARCHS="$(lipo -archs "$APP/Contents/MacOS/AIRemoteMac")"
case "$ARCHS" in
...
<plist version="1.0">
<dict>
    <key>CFBundleName</key>              <string>AIRemote for Mac</string>
    <key>CFBundleDisplayName</key>       <string>AIRemote for Mac</string>
    <!-- ⚠️ 0.7.0 从 com.meiease.joycoding 改过来。macOS 的辅助功能授权是按
         bundle id 记的, 换了 id 等于换了一个 app —— 老用户必须重新授权一次,
         而且"系统设置 → 辅助功能"列表里会同时留着旧的那条(可以手动删)。 -->
    <key>CFBundleIdentifier</key>        <string>com.meiease.airemote.mac</string>
    <key>CFBundleExecutable</key>        <string>AIRemoteMac</string>
...
if [[ "$JOYPAD_SIGN_ID" == "Developer ID Application"* && "$WANT_NOTARIZE" == 1 ]]; then
  if security find-generic-password -s "com.apple.gke.notary.tool" >/dev/null 2>&1 \
    ...
    ZIP="build/AIRemote-for-Mac.zip"
    ...
    if xcrun notarytool submit "$ZIP" --keychain-profile notary --wait; then
      ...
      rm -f "$ZIP"
      # 重新打一个已钉票的分发包
      ditto -c -k --keepParent "$APP" "build/AIRemote-for-Mac-notarized.zip"
      echo "✅ 可分发: build/AIRemote-for-Mac-notarized.zip"
      ...
      if [[ "$WANT_DMG" == 1 ]]; then
        DMG="build/AIRemote-for-Mac-$VERSION-universal.dmg"
        echo "▸ 打 DMG"
        packaging/make-dmg.sh "$APP" "$DMG" "AIRemote for Mac"
        echo "▸ 签名 DMG"
